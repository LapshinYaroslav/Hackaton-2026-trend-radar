...
Порядок признаков — model.config.FEATURES, тот же, в котором лежат коэффициенты.
...
def features_from_counters(counters: pd.DataFrame,
                           totals: pd.DataFrame | None = None) -> dict[str, float]:
    """Шесть признаков модели по годовым счётчикам одной технологии.

    Вход — счётчики до агрегации: по рабочим окнам возраст первого упоминания
    не определить, они перекрываются.
    """
...
def candidate_features(name: str, *, terms: list[str] | None = None,
                       ...
                       area: str = "", fetch: Fetch | None = None) -> dict:
    ...
    Возвращает словарь:
      ...
      features     шесть чисел в порядке model.config.FEATURES;
    ...
    answer = {"name": name, "area": area, "query": query, "terms": list(search.terms),
              ...
              "features": {name: math.nan for name in FEATURES}, "complete": False}
    if counters.empty:
        return answer
    answer["features"] = features_from_counters(counters)
